...
def box_space_to_json(box_space: gym.spaces.Box) -> dict:
    # Check if the Box space is bounded by infinities
    if np.all(box_space.low == -np.inf):
        low = "-inf"
    elif box_space.low.size == 1:
        low = box_space.low.item()
    else:
        low = box_space.low.tolist()
    
    if np.all(box_space.high == np.inf):
        high = "inf"
    elif box_space.high.size == 1:
        high = box_space.high.item()
    else:
        high = box_space.high.tolist()

    shape = list(box_space.shape)
    dtype = str(box_space.dtype)
...
class BlackboxRunner:
    def __init__(self, args, algo_args, env_args):
        ...
        self.black_query_uri = args["black_query"]
        # 先连接
        connect_data = {
            "obs_space": box_space_to_json(self.envs.observation_space[0]),
            "act_space": box_space_to_json(self.envs.action_space[0]),
            "num_agents": self.num_agents,
        }
        print(connect_data)
        res = requests.post(self.black_query_uri + "/connect", json=connect_data, timeout=10)
        # 获取res中的token
    ...
    def query_perform(self, agent_id, eval_obs, eval_masks, eval_available_actions):
        data = {
            "token": self.token,
            "agent_id": agent_id,
            "eval_obs": eval_obs[:, agent_id].tolist(), 
            "eval_masks": eval_masks[:, agent_id].tolist(), 
            "eval_available_actions": eval_available_actions[:, agent_id].tolist() if eval_available_actions[0] is not None else None
        }
        res = requests.post(self.black_query_uri + "/perform", json=data, timeout=10).json()
        return res["actions"]
    ...
    def run(self):
        ...
        if self.args["attack"] == "random_noise":
            ...
            raise ValueError("黑盒模式不支持该对抗攻击方式: ", self.args["attack"])
    ...
    def eval(self):
        ...
        while True:
            ...
            for agent_id in range(self.num_agents):
                eval_actions = self.query_perform(agent_id, eval_obs, eval_masks, eval_available_actions)
    ...
    def eval_blackbox_perturbation(self):
        ...
        while True:
            ...
            if self.algo_args["train"]["targeted_attack"]:
                ...
                for agent_id in range(self.num_adv_agents):
                    eval_adv_actions = self.query_perform(agent_id, gather(eval_obs, adv_agent_ids, axis=1), gather(eval_masks, adv_agent_ids, axis=1), gather(eval_available_actions, adv_agent_ids, axis=1))
            ...
            eval_actions_collector = []
            for agent_id in range(self.num_agents):
                eval_actions = self.query_perform(agent_id, eval_obs_adv, eval_masks, eval_available_actions)
                eval_actions_collector.append(_t2n(eval_actions))
            eval_actions = np.array(eval_actions_collector).transpose(1, 0, 2)
    ...
    def eval_random_noise(self):
        ...
        self.eval_blackbox_perturbation()
