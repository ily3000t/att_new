...
class OnPolicyRunner(BaseRunner):
    ...
    def train(self, next_values):
        ...
        actor_train_infos = []
        if self.share_param:
            actor_train_info = self.algo.share_param_train_actor(self.buffers) if not self.algo_args['train'].get('use_regulation', False) else self.algo.train_actor(self.buffers, ernie_regularizers)
            for _ in range(self.num_agents):
                actor_train_infos.append(actor_train_info)
        else:
            #print(f"ernie_regularizers: {len(ernie_regularizers)}, {ernie_regularizers}\n buffers: {len(self.buffers)}, {self.buffers}")    
            for agent_id in range(self.num_agents):
                actor_train_info = self.algo.train_actor(self.buffers[agent_id], agent_id) if not self.algo_args['train'].get('use_regulation', False) else self.algo.train_actor(self.buffers[agent_id], agent_id, ernie_regularizers[agent_id])
                actor_train_infos.append(actor_train_info)
