...
class OnPolicyRunner(BaseRunner):
    def __init__(self, args, algo_args, env_args):
        ...
        self.fixed_order = args["fixed_order"] if "fixed_order" in args else False
        ...
        if self.algo_args["train"]["use_render"] is False:  # train, not render
            ...
            for agent_id in range(self.num_agents):
                scheme = {
                    ...
                    "rnn_states_actor": {
                        "vshape": (self.recurrent_n, self.rnn_hidden_size),
                        "offset": 1,
                        "extra": ["rnn_state"],
                    },
                    "share_obs": {
                        "vshape": get_shape_from_obs_space(self.envs.share_observation_space[0]),
                        "offset": 1,
                    },
                    "rnn_states_critic": {
                        "vshape": (self.recurrent_n, self.rnn_hidden_size),
                    ...
                    "action_log_probs": {
                        "vshape": (get_shape_from_act_space(self.envs.action_space[agent_id]),),
                        "offset": 0,
                    },
                    ...
                    "factor": {"vshape": (1,), "offset": 1},
                }
                if self.action_type == "Discrete":
                    scheme["available_actions"] = {
                        "vshape": (self.envs.action_space[agent_id].n,),
                        "offset": 1,
                        "init_value": 1,
                    }
    ...
    def init_batch(self):
        ...
        for agent_id in range(self.num_agents):
            data = {"obs": obs[:, agent_id].copy(), "share_obs": share_obs[:, agent_id].copy()}
    ...
    def run(self):
        ...
        for episode in range(1, episodes + 1):
            ...
            self.current_timestep = (
                episode * self.algo_args["train"]["episode_length"] * self.algo_args["train"]["n_rollout_threads"]
            )
            ...
            if episode % self.algo_args["train"]["eval_interval"] == 0:
                ...
                if "slice" in self.algo_args["train"] and "slice_interval" in self.algo_args["train"] and save_count % self.algo_args['train']['slice_interval'] == 0:
    ...
    def train(self, next_values):
        ...
        for agent_id in range(self.num_agents):
            ...
            if self.value_normalizer is not None:
                advantage = (
                    self.buffers[agent_id].data["returns"][:, :-1]
                    - self.value_normalizer.denormalize(self.buffers[agent_id].data["value_preds"])[:, :-1]
                )
            else:
                advantage = (
                    self.buffers[agent_id].data["returns"][:, :-1] - self.buffers[agent_id].data["value_preds"][:, :-1]
                )
        ...
        # 计算 ERNIE 对抗正则项
        ernie_regularizers = []
        use_regulation = self.algo_args['train'].get("use_regulation", False)

        if use_regulation:
            epsilon = self.algo_args.get("ernie_epsilon", 0.1)  # 扰动强度参数
            for agent_id in range(self.num_agents):
                # 从 buffer 中获取观察值
                obs_batch = self.buffers[agent_id].data["obs"][:, :-1].reshape(-1, self.buffers[agent_id].data["obs"].shape[-1])
                masks_batch = self.buffers[agent_id].data["masks"][:, :-1].reshape(-1, 1)
                rnn_states_batch = self.buffers[agent_id].data["rnn_states_actor"][:, :-1].reshape(-1, self.recurrent_n, self.rnn_hidden_size)
                
                # 计算对抗样本和正则项
                if "available_actions" in self.buffers[agent_id].data:
                    avail_actions_batch = self.buffers[agent_id].data["available_actions"][:, :-1].reshape(-1, self.buffers[agent_id].data["available_actions"].shape[-1])
                    ernie_reg = self.algo.compute_ernie_regularizer(obs_batch, rnn_states_batch, masks_batch, avail_actions_batch, agent_id, epsilon)
                else:
                    ernie_reg = self.algo.compute_ernie_regularizer(obs_batch, rnn_states_batch, masks_batch, None, agent_id, epsilon)
        ...
        # first agent's factor is all ones
        factor = np.ones(
            (
                self.algo_args["train"]["n_rollout_threads"],
                self.algo_args["train"]["episode_length"],
        ...
        # delete share_param and add order
        if self.fixed_order:
        ...
        for agent_id in agent_order:
            if self.algo_args['train'].get("use_regulation", False):
                ernie_regularizer = ernie_regularizers[agent_id] if use_regulation else None

            # caculate old_probs
            actor_train_info, factor = self.algo.train_actor(self.buffers[agent_id], agent_id, factor) if not self.algo_args['train'].get('use_regulation', False) else self.algo.train_actor(self.buffers[agent_id], agent_id, factor, ernie_regularizer)
