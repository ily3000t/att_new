...
class BaseRunner:
    def __init__(self, args, algo_args, env_args):
        ...
        if self.algo_args['train']['use_render']:  # make envs for rendering
            ...
            # 使用num_env_steps判断是否需要训练，如果不需要训练，则不创建训练环境，避免类似dexhands、network等环境无法创建多个环境实例的问题
            if args["env"] == "dexhands" and self.algo_args['train']["use_eval"] is True:
                # TODO 这种方法只能解决一部分问题，类似adaptive action这种需要训练的攻击方法还是无法使用
        ...
        self.num_adv_agents = self.num_agents if algo_args["train"]["adv_all"] \
                                  else len(algo_args["train"]["adv_agent_ids"])
    ...
    def get_certain_adv_ids(self):
        if self.algo_args["train"]["adv_all"]:
            adv_agent_ids = [t for t in range(self.num_agents)]
        else:
    ...
    # 不攻击的话就调成0
    def whether_perturbation_eps(self):
        eps = self.algo_args["train"]["perturbation_eps"]
        random_number = random.random()
        # print(f"eps is {random_number}")
        return random_number < eps

    @torch.no_grad()
    def eval(self):
        """Evaluate the model. All algorithms should fit this evaluation pipeline."""
        print(50 * "-")
        print("n_eval_rollout_threads: ", self.n_eval_rollout_threads)
        print("eval_episodes: ", self.algo_args["train"]["eval_episodes"])
        print("eval_recover_mode: ", self.algo_args["extra"]["eval_recover_mode"] if "eval_recover_mode" in self.algo_args["extra"] else False)
        print("recover_episode: ", self.algo_args["extra"]["recover_episode"] if "recover_episode" in self.algo_args["extra"] else int(self.algo_args["train"]["eval_episodes"] / 2))
        print(50 * "-")
        ...
        while True:
            ...
            self.logger.eval_per_step(eval_data, "eval_per_step_rewards")  # logger callback at each step of evaluation
    ...
    def eval_adv(self):
        ...
        while True:
            ...
            # if execute_recover, then do not attack
            execute_recover = False
            if "eval_recover_mode" in self.algo_args["extra"] and self.algo_args["extra"]["eval_recover_mode"]:
                recover_episode = self.algo_args["extra"]["recover_episode"] if "recover_episode" in self.algo_args["extra"] else int(self.algo_args["train"]["eval_episodes"] / 2)
                execute_recover = eval_episode >= recover_episode
                if execute_recover:
                    self.logger.eval_log_recovery_timestep()

            # 如果不执行恢复模式，则添加攻击
            if not execute_recover:

                if "random_action_attack" in self.algo_args["extra"] and self.algo_args["extra"]["random_action_attack"]:
                    # 执行的是 random_action 攻击，需要配合adv_agent_ids来指定攻击的agent
                    eval_adv_actions = np.array([[self.envs.action_space[0].sample() for _ in range(len(adv_agent_ids[0]))] for _ in range(len(adv_agent_ids))])
                    eval_actions = scatter(eval_actions, adv_agent_ids, eval_adv_actions, axis=1)

                else: # 执行的是默认perturbation攻击
                    # 2. If targeted attack, generate adversarial actions
                    # 如果有训练好的内鬼，则直接生成对抗性动作
                    # 目前全部traitor配置中的targeted_attack = False
                    ...
                    R_pi_values = []  # 保存每个 agent 的 R_π 值
                    for agent_id in range(self.num_agents):
                        # 原始观测和扰动观测
                        obs_original = eval_obs[:, agent_id]       # [n_threads, obs_dim]
                        obs_perturbed = eval_obs_adv[:, agent_id] # [n_threads, obs_dim]

                        # 转换为 Tensor
                        obs_original_tensor = torch.FloatTensor(obs_original).to(self.device)
                        obs_perturbed_tensor = torch.FloatTensor(obs_perturbed).to(self.device)

                        # 获取 RNN 状态和 masks
                        rnn_states_agent = torch.FloatTensor(eval_rnn_states[:, agent_id]).to(self.device)
                        masks_agent = torch.FloatTensor(eval_masks[:, agent_id]).to(self.device)

                        # 获取动作分布
                        with torch.no_grad():
                            action_dist_original = self.victims[agent_id].get_action_distribution(
                                obs_original_tensor,
                                rnn_states_agent,
                                masks_agent,
                                eval_available_actions[:, agent_id] if eval_available_actions[0] is not None else None
                            )
                            action_dist_perturbed = self.victims[agent_id].get_action_distribution(
                                obs_perturbed_tensor,
                                rnn_states_agent,
                                masks_agent,
                                eval_available_actions[:, agent_id] if eval_available_actions[0] is not None else None
                            )
                        ...
                        if self.action_type == "Discrete":
                            divergence = torch.distributions.kl.kl_divergence(
                                action_dist_perturbed, action_dist_original
                            ).mean().item()
                        else:
                    ...
                    self.logger.eval_log_Rpi(R_pi_dict)

                    # 4. Get actions after attack (update rnn hidden states)
                    eval_actions_collector = []
                    for agent_id in range(self.num_agents):
                        eps_perform = self.victims[agent_id].perform
                        if self.whether_perturbation_eps():
                            if not self.algo_args['extra'].get("use_mad_perform", False):
                                eps_perform = self.victims[agent_id].worst_perform
                                # print(f"use worst perform {agent_id}")
                            else:
                                eps_perform = self.victims[agent_id].mad_perform
                                # print(f"use mad perform {agent_id}")
                        eval_actions, temp_rnn_state = eps_perform(
            ...
            self.logger.eval_per_step(eval_data, "eval_adv_per_step_rewards")  # logger callback at each step of evaluation
    ...
    def eval_slice(self):
        ...
        for folder_name in pth_dir_list:
            ...
            if os.path.isdir(victim_model_dir):
                ...
                self.algo_args["extra"]["eval_recover_mode"] = False
                self.eval_adv()
                self.logger.eval_log_slice("adv", folder_name)
                self.algo_args["extra"]["eval_recover_mode"] = True
                self.eval_adv()
                self.logger.eval_log_slice("resilience", folder_name)
