...
class OnPolicyRunner(BaseRunner):
    ...
    def run(self):
        ...
        # only train the adversary when the number of episodes is not 0
        if episodes != 0:
            ...
            for step in range(self.algo_args['train']['episode_length']):
                ...
                for agent_id in range(self.num_agents):
                    ...
                    if self.whether_perturbation_eps():
                        ...
                        # print("not use worst")
                        pass
                    actions, temp_rnn_state = eps_perform(
                        obs_adv[:, agent_id],
                        rnn_states[:, agent_id],
                        masks[:, agent_id],
                        available_actions[:, agent_id]
                    ...
                    actions_collector.append(_t2n(actions))
                # print("actions_collector is: ", np.array(actions_collector))
                # print()
        ...
        # final evaluation
        if self.algo_args['train']['use_eval']:
            log_resilience_reward_path = self.algo_args["extra"].get("eval_recover_log_path", None)
            eval_recover_mode_ori = self.algo_args["extra"].get("eval_recover_mode", False)
            self.logger.episode_init(self.current_timestep)
            
            self.eval()
            self.logger.eval_log_slice("vanilla", "final")
            eval_avg_rew_coop = np.mean(self.logger.eval_episode_rewards)
            
            if log_resilience_reward_path is not None:
                self.algo_args["extra"]["eval_recover_mode"] = False
            self.eval_adv()
            self.logger.eval_log_slice("adv", "final")
            eval_avg_rew_adv = np.mean(self.logger.eval_episode_rewards)
            
            if log_resilience_reward_path is not None:
                self.algo_args["extra"]["eval_recover_mode"] = True
                self.eval_adv()
                self.logger.eval_log_slice("resilience", "final")
                eval_avg_rew_res = np.mean(self.logger.eval_episode_rewards)

                trick = self.algo_args["extra"].get("resilience_log_trick", "")
                attack_method = self.algo_args["extra"].get("resilience_log_attack_method", "")
                import csv
                with open(log_resilience_reward_path, 'a', newline='', encoding='utf-8') as file:
                    writer = csv.writer(file)
                    writer.writerow([trick, attack_method, eval_avg_rew_coop, eval_avg_rew_adv, eval_avg_rew_res])
            self.algo_args["extra"]["eval_recover_mode"] = eval_recover_mode_ori
            if 'slice' in self.algo_args['train'] and self.algo_args['train']['slice']:
                self.eval_slice()
