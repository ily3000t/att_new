...
class HAPPO:
    ...
    def update_actor(self, sample, agent_id, ernie_regularizer=None):
        ...
        active_masks = check(active_masks).to(**self.tpdv)
        factor = check(factor).to(**self.tpdv)
        ...
        if self.use_policy_active_masks:
            policy_action_loss = (
                -torch.sum(factor * torch.min(surr1, surr2), dim=-1, keepdim=True) * active_masks
            ).sum() / active_masks.sum()
        else:
            policy_action_loss = -torch.sum(factor * torch.min(surr1, surr2), dim=-1, keepdim=True).mean()

        policy_loss = policy_action_loss

        # 加入 ERNIE 正则项
        if ernie_regularizer is not None and self.args.get("use_regulation", False):
    ...
    def update_critic(self, sample, value_normalizer=None):
        ...
        value_loss = self.cal_value_loss(values, value_preds, returns, value_normalizer=value_normalizer)
        ...
        for _ in range(self.critic_epoch):
            ...
            for agent_id in range(self.num_agents):
                if self.use_recurrent_policy:
                    data_generator = buffers[agent_id].chunk_generator(
                        self.actor_num_mini_batch, self.data_chunk_length
                    )
                else:
    ...
    def train_actor(self, buffer, agent_id, factor, ernie_regularizer=None):
        train_info = {}
        ...
        # first update factor into actor buffer
        buffer.update_factor(factor)  # current actor save factor

        # before training actor, calculate old probs
        data_generator = buffer.step_generator(self.actor_num_mini_batch)
        pre_sample = next(data_generator)

        old_actions_logprob, _ = self.evaluate_actions(
            agent_id,
            pre_sample["obs"],
            pre_sample["rnn_states_actor"],
            pre_sample["actions"],
            pre_sample["masks"],
            pre_sample["available_actions"] if "available_actions" in pre_sample else None,
            pre_sample["active_masks"],
        )
        ...
        # after training actor, calculate new probs
        new_actions_logprob, _ = self.evaluate_actions(
            agent_id,
            pre_sample["obs"],
            pre_sample["rnn_states_actor"],
            pre_sample["actions"],
        ...
        return train_info, new_factor

    def share_param_train_actor(self, buffers):
        raise Exception("HAPPO don't support share actors' param.")
