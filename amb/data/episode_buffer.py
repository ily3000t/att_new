...
class EpisodeBuffer:
    ...
    def insert(self, data, t):
        ...
        for key in data:
            ...
            if type(d[0]) == list:
                d = np.array([np.array(inner_list, dtype=dtype) for inner_list in d])
            # print(str(type(d[0]))+" "+str(dtype)+" "+str(d))
            d = d.astype(dtype)
            #print(f"d.shape: {d.shape}, expected shape: {self.scheme[key]['vshape']}")
            #print(f"n: {n}, vshape: {self.scheme[key]['vshape']}, d.shape: {d.shape}")
            if d.shape[0] < n:
                padding_rows = n - d.shape[0]
                d = np.pad(d, ((0, padding_rows), (0, 0)), mode='constant', constant_values=0)
            ...
            if self.current_size + n > self.buffer_size:
                ...
                if "sample_next" in extra:
                    ...
                    shape1 = self.data["next_" + key][:right, max(t+offset-self.n_step, 0)].shape
                    shape2 = d[:, np.newaxis].shape
                    if len(shape1) == len(shape2):
                        self.data["next_" + key][:right, max(t+offset-self.n_step, 0)] = d[left:, np.newaxis].copy()
                    else:
                        self.data["next_" + key][:right, max(t+offset-self.n_step, 0)] = d[left:, np.newaxis].squeeze(axis=1).copy()
            else:
    ...
    def init_batch(self, data):
        ...
        def pad_array(array, target_length):
            ...
            if array.shape[0] < target_length:
                pad_width = target_length - array.shape[0]
                array = np.pad(array, (0, pad_width), mode='constant', constant_values=0)
            return array
