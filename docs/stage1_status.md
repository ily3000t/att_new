...
1. 扩展独立训练 seeds 2、3，并对 Slow-down 执行同样的曲线记录；依据实际曲线决定是否增加训练预算，不设置零碰撞或固定改善比例门槛。
2. 记录有限预算下的平台行为、未收敛情况和不同 seed 的差异。当前 snapshot 不含完整续训状态，增加预算的运行必须明确标为新训练，或先实现并验证完整恢复。
3. 固定各 victim，继续原攻击预算/作用对象对照，增加实际动作改变率和计算次数，审计 targeted-observation、动作攻击和 MAD 的实现语义。保留无效攻击和攻击反而提高回报的结果。
4. 在明确威胁模型后实现 Ours 的薄接口与风险目标，比较同预算下的回报损失、碰撞率变化及其他安全指标；分别报告 native CACC 与后续 SUMO 任务的结果。
