...
ATTACK_CONF = {
    # Observation uncertainty: Gaussian-single
    ...
    # Observation uncertainty: Greedy-single, eps!=1
    ...
    "pert_act": "--run traitor --algo.num_env_steps 0 --algo.perturb_iters 0 --algo.adaptive_alpha False --algo.targeted_attack False --perturbation_eps 0.2",    # Observation uncertainty; 
    # Observation uncertainty: Optimal-single, eps=1
    ...
    # Observation uncertainty: Greddy-all
...
def _write_train_command(file, config_path, exp_name, logs_dir, trick_str="", trick=None, gpu=0, extra=None):
    ...
    command = f"CUDA_VISIBLE_DEVICES={gpu} python -u {TRIAN_ENTRY_PATH} --load_config {config_path} --exp_name {exp_name} {trick_str} {'--headless' if extra=='headless' else extra if extra else ''} > {ld}/train.log 2>&1"
    file.write(command + "\n")
...
def generate_eval_scripts(args):
    ...
    with open(file_name, "w") as f:
        ...
        for attack_method, attack_cfg in ATTACK_CONF.items() if stage != 2 else ATTACK_CONF_STAGE_2.items():
            ...
            for victim_dir, sub_victim_dir in victim_tuples:
                ...
                if stage == 1 and attack_method in ["adaptive_action", "traitor", "learn_act", "learn_obs_all", "learn_obs_sin", "learn_act_all"]:
                    if victim_dir != "default":
                        ...
                        if attack_method == "learn_act_all": # learn_act_all必须放在stage1最后一个
                            base_cfg += " --algo.n_eval_rollout_threads 20"
                            base_cfg = base_cfg.replace("--algo.use_eval False", "--algo.use_eval True")

                if stage == 2:
                    if attack_method in ["adaptive_action", "traitor", "learn_act", "learn_obs_all", "learn_obs_sin", "learn_act_all"]:
                        type = "traitor" if attack_method in ["traitor", 'learn_act_all', 'learn_act'] else "perturbation"
                        ...
                        if trick is not None and trick != victim_dir:
                            continue
                        command = f"CUDA_VISIBLE_DEVICES={gpu} python -u {TRIAN_ENTRY_PATH} {base_cfg} --algo.slice {slice if victim_dir != 'default' else True} --algo.model_dir {latest_models_dir} --load_victim {os.path.join(models_dir, victim_dir, sub_victim_dir)} --exp_name {attack_method}_{victim_dir} {attack_cfg} {extra if extra else ''} > {logs_dir}/{attack_method}.log 2>&1"
                ...
                if trick is not None and trick != victim_dir:
                    continue
                command = f"CUDA_VISIBLE_DEVICES={gpu} python -u {TRIAN_ENTRY_PATH} {base_cfg} --algo.slice {slice if victim_dir != 'default' else True} --load_victim {os.path.join(models_dir, victim_dir, sub_victim_dir)} --exp_name {attack_method}_{victim_dir} {attack_cfg} {extra if extra else ''} > {logs_dir}/{attack_method}.log 2>&1"
                f.write(command + "\n")
            f.write("\n")
        f.write("\n")
    ...
    print(f"python parallel.py -s {file_name} -o {out_dir} -n {num_workers}")
    if stage == 1:
        print(f"python generate.py eval -e {env} -s {scenario} -a {algo} --stage 2" + (f" --extra='--headless'" if extra == "--headless" else ""))
